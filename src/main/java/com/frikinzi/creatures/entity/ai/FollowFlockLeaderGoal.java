...
public class FollowFlockLeaderGoal extends Goal {
    ...
    public boolean canUse() {
        if (this.mob.hasFollowers()) {
            ...
            CreaturesBirdEntity flockbirds = DataFixUtils.orElse(list.stream().filter(CreaturesBirdEntity::canBeFollowed).findAny(), this.mob);
            flockbirds.addFollowers(list.stream().filter((p_212823_0_) -> {
