...
public class WhistlingDuckEntity extends CreaturesWalkingBird implements GeoEntity {
    ...
    private static final Ingredient FOOD_ITEMS = Ingredient.of(Items.PUMPKIN_SEEDS, Items.WHEAT_SEEDS, Items.BEETROOT_SEEDS, Items.SEAGRASS, Items.KELP, Items.GRASS);
    ...
    public static final Map<Integer, Component> SPECIES_NAMES = ImmutableMap.<Integer, Component>builder()
            .put(1, Component.translatable("message.creatures.fulvous"))
            .put(2, Component.translatable("message.creatures.whitefacedwhistling"))
            .put(3, Component.translatable("message.creatures.blackbelliedwhistling"))
            .put(4, Component.translatable("message.creatures.lesserwhistling"))
            .build();
    ...
    protected <E extends WhistlingDuckEntity> PlayState walkAnimController(final AnimationState<E> event) {
        if (this.isBaby()) {
            if (event.isMoving()) {
    ...
    @Override
    public WhistlingDuckEntity getBreedOffspring(ServerLevel p_241840_1_, AgeableMob p_241840_2_) {
        WhistlingDuckEntity wildduckentity = CreaturesEntities.WHISTLING_DUCK.get().create(p_241840_1_);
    ...
    public SoundEvent getAmbientSound() {
        if (!this.isSleeping()) {
            if (this.getVariant() == 2) {
                return CreaturesSound.WHITE_FACED.get();
            }
            return CreaturesSound.WHISTLING_DUCK.get();
        }
        else
        {
            return null;
    ...
    public double getHatchChance() {
        return CreaturesConfig.whistlingduck_hatch_chance.get();
    }
    ...
    public Component getFunFact() {
        return Component.translatable("description.creatures.whistlingduck");
    }
