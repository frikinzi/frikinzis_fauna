...
public class CreaturesBirdEntity extends ShoulderRidingEntity {
    ...
    protected void defineSynchedData() {
        ...
        entityData.define(WANDERING, 0);
        entityData.define(HEIGHT_MULTIPLIER, 1.0F);
        entityData.define(DATA_FLAGS_ID, (byte)0);
        entityData.define(GROOMING, false);
    }
    ...
    public Component getGenderText() {
        if (this.getGender() == 1) {
            return Component.translatable("gui.male");
        } return Component.translatable("gui.female");
    }

    public void setVariant(int p_29449_) {
        entityData.set(DATA_VARIANT_ID, p_29449_);
    }
    ...
    public String getHeightString() {
        if (this.getHeightMultiplier() >= 1.5) {
            Component i = Component.translatable("gui.giant");
            return i.getString();
        }
        if (this.getHeightMultiplier() >= 1.4) {
            Component i = Component.translatable("gui.huge");
            return i.getString();
        }
        if (this.getHeightMultiplier() >= 1.21) {
            Component i = Component.translatable("gui.large");
            return i.getString();
        } if (this.getHeightMultiplier() < 1.21 && this.getHeightMultiplier() > 1.11) {
            Component i = Component.translatable("gui.above_average");
            return i.getString();
        }
        if (this.getHeightMultiplier() <= 1.11 && this.getHeightMultiplier() >= 0.89) {
            Component i = Component.translatable("gui.average");
            return i.getString();
        }
        if (this.getHeightMultiplier() < 0.89 && this.getHeightMultiplier() >= 0.79) {
            Component i = Component.translatable("gui.below_average");
            return i.getString();
        }
        else {
            Component i = Component.translatable("gui.small");
            return i.getString();
        }
    }
...
//            }
    ...
    public InteractionResult mobInteract(Player p_29414_, InteractionHand p_29415_) {
        ...
        if (!this.isTame() && this.isFood(itemstack) && this.canTame()) {
            ...
            if (this.isWandering() == 0) {
                if (this.level().isClientSide) {
                    Component i = Component.translatable("message.wander");
    ...
    public static class BirdData extends AgeableMob.AgeableMobGroupData {
    ...
    public ChatFormatting getIUCNColor() {
        if (this.getIUCNStatus() == 0) {
            return ChatFormatting.DARK_GREEN; // least concern
        } if (this.getIUCNStatus() == 1) {
            return ChatFormatting.GOLD; // near threatened
        } if (this.getIUCNStatus() == 2) {
            return ChatFormatting.GOLD;  // vulnerable
        } if (this.getIUCNStatus() == 3) {
            return ChatFormatting.RED; // endangered
        } if (this.getIUCNStatus() == 4) {
            return ChatFormatting.DARK_RED; // critically endangered
        } if (this.getIUCNStatus() == 5) {
            return ChatFormatting.DARK_PURPLE; // extinct in the wild
        } if (this.getIUCNStatus() == 6) {
            return ChatFormatting.BLACK; // extinct
        } return ChatFormatting.GRAY; // unknown
    }

    public Component getIUCNText() {
        if (this.getIUCNStatus() == 0) {
            return Component.translatable("creatures.leastconcern"); // least concern
        } if (this.getIUCNStatus() == 1) {
            return Component.translatable("creatures.nearthreatened"); // near threatened
        } if (this.getIUCNStatus() == 2) {
            return Component.translatable("creatures.vulnerable");  // vulnerable
        } if (this.getIUCNStatus() == 3) {
            return Component.translatable("creatures.endangered"); // endangered
        } if (this.getIUCNStatus() == 4) {
            return Component.translatable("creatures.criticallyendangered"); // critically endangered
        } if (this.getIUCNStatus() == 5) {
            return Component.translatable("creatures.extinctinwild"); // extinct in the wild
        } if (this.getIUCNStatus() == 6) {
            return Component.translatable("creatures.extinct"); // extinct
        } return Component.translatable("creatures.datadeficient"); // unknown
    }
