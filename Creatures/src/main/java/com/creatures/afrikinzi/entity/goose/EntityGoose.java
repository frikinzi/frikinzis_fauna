...
public class EntityGoose extends AbstractCreaturesNonTameable implements IAnimatable, EntityFlying, ICreaturesEntity {
    private AnimationFactory factory = new AnimationFactory(this);
    private static final DataParameter<Integer> VARIANT = EntityDataManager.<Integer>createKey(EntityGoose.class, DataSerializers.VARINT);
    protected static final DataParameter<Boolean> SLEEPING = EntityDataManager.createKey(EntityGoose.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> GENDER = EntityDataManager.<Integer>createKey(EntityGoose.class, DataSerializers.VARINT);
    private static final Set<Item> TAME_ITEMS = Sets.newHashSet(new ItemStack(Blocks.GRASS).getItem(), new ItemStack(Blocks.TALLGRASS).getItem());
    public float flap;
    ...
    public EntityGoose(World worldIn)
    {
        super(worldIn);
    ...
    public boolean canMateWith(EntityAnimal otherAnimal)
    {
        ...
        else if (!(otherAnimal instanceof EntityGoose))
        {
            return false;
        }
        else
        {
            EntityGoose entityibis = (EntityGoose)otherAnimal;
    ...
    {
        EntityGoose entityibis = new EntityGoose(this.world);
    ...
    public SoundEvent getAmbientSound()
    {
        if (!this.isSleeping()) {
            if (this.isFlying()) {
                return SoundsHandler.GOOSE_AMBIENT_FLY;
            }
            return SoundsHandler.GOOSE_AMBIENT;
        } else {
            return null;
        }
    }
    ...
    public String getSpeciesName() {
        if (this.getVariant() == 1) {
            String s1 = I18n.format("message.creatures.canada");
            return s1;
        }
        else if (this.getVariant() == 2) {
            String s1 = I18n.format("message.creatures.barnacle");
            return s1;
        }
        else if (this.getVariant() == 3) {
            String s1 = I18n.format("message.creatures.graylag");
            return s1;
        }
        else if (this.getVariant() == 4) {
            String s1 = I18n.format("message.creatures.snow");
            return s1;
        }
        else if (this.getVariant() == 5) {
            String s1 = I18n.format("message.creatures.orinoco");
            return s1;
        }
        else {
            return "Unknown";
