...
public class EntityRobin extends AbstractCreaturesNonTameable implements IAnimatable, EntityFlying, ICreaturesEntity {
    private AnimationFactory factory = new AnimationFactory(this);
    private static final DataParameter<Integer> VARIANT = EntityDataManager.<Integer>createKey(EntityRobin.class, DataSerializers.VARINT);
    protected static final DataParameter<Boolean> SLEEPING = EntityDataManager.createKey(EntityRobin.class, DataSerializers.BOOLEAN);
    ...
    public EntityRobin(World worldIn)
    {
        super(worldIn);
    ...
    public boolean canMateWith(EntityAnimal otherAnimal)
    {
        ...
        else if (!(otherAnimal instanceof EntityRobin))
        {
            return false;
        }
        else
        {
            EntityRobin entityroller = (EntityRobin)otherAnimal;
            if (this.getGender() == entityroller.getGender()) {
                return false;
            } else {
                return this.isInLove() && entityroller.isInLove();
            }
        }
    ...
    {
        EntityRobin entityroller = new EntityRobin(this.world);
    ...
    public SoundEvent getAmbientSound()
    {
        if (!this.isSleeping()) {
            ...
            return SoundsHandler.ROBIN_AMBIENT;
        } else {
            return null;
        }
    }
    ...
    public String getSpeciesName() {
        if (this.getVariant() == 1) {
            String s1 = I18n.format("message.creatures.europeanrobin");
            return s1;
        }
        else if (this.getVariant() == 2) {
            String s1 = I18n.format("message.creatures.roserobin");
            return s1;
        }
        else if (this.getVariant() == 3) {
            String s1 = I18n.format("message.creatures.easternyellow");
            return s1;
        }
        else if (this.getVariant() == 4) {
            String s1 = I18n.format("message.creatures.japaneserobin");
            return s1;
        }
        else if (this.getVariant() == 5) {
            String s1 = I18n.format("message.creatures.scarletrobin");
            return s1;
        } else {
            return "Unknown";
        }
