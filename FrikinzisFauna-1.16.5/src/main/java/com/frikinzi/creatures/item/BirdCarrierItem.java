...
public class BirdCarrierItem extends Item {
    ...
    public ActionResultType interactLivingEntity(ItemStack stack, PlayerEntity player, LivingEntity target, Hand hand) {
        if (target instanceof CreaturesBirdEntity) {
            if (stack.hasTag()) {
                player.displayClientMessage(new TranslationTextComponent("creatures.message.bird_carrier.full"), true);
                ...
                target.remove();
                player.displayClientMessage(new TranslationTextComponent("creatures.message.bird_carrier.retrieve"), true);
    ...
    public ActionResultType useOn(ItemUseContext context) {
        ...
        if (!item.hasTag()) {
            player.displayClientMessage(new TranslationTextComponent("creatures.message.bird_carrier.empty"), true);
            return ActionResultType.PASS;
        }
        if (!world.isClientSide) {
            ...
            if (entity instanceof CreaturesBirdEntity) {
                ...
                player.displayClientMessage(new TranslationTextComponent("creatures.message.bird_carrier.released"), true);
            }
