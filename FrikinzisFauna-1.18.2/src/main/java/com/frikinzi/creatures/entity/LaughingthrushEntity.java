...
public class LaughingthrushEntity extends CreaturesFlyingBirdEntity implements IAnimatable, IAnimationTickable {
    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
    public static Map<Integer, TranslatableComponent> SPECIES_NAMES = ImmutableMap.of(
            1, new TranslatableComponent("message.creatures.chinesehua"),
            2, new TranslatableComponent("message.creatures.whitecrested"),
            3, new TranslatableComponent("message.creatures.chinesehua"),
            4, new TranslatableComponent("message.creatures.blackhooded"),
            5, new TranslatableComponent("message.creatures.bluecrowned"),
            6, new TranslatableComponent("message.creatures.whitecheeked"),
            7, new TranslatableComponent("message.creatures.redtailed")
    );

    public LaughingthrushEntity(EntityType<? extends LaughingthrushEntity> p_29362_, Level p_29363_) {
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<LaughingthrushEntity>(this, "controller", 0, this::predicate));
    }
    ...
    public static boolean checkBirdSpawnRules(EntityType<LaughingthrushEntity> p_29424_, LevelAccessor p_29425_, MobSpawnType p_29426_, BlockPos p_29427_, Random p_29428_) {
    ...
    public LaughingthrushEntity getBreedOffspring(ServerLevel p_149088_, AgeableMob p_149089_) {
        LaughingthrushEntity thrush = CreaturesEntities.LAUGHINGTHRUSH.get().create(p_149088_);
        thrush.setVariant(this.getVariant());
        thrush.setGender(this.random.nextInt(2));
        thrush.setHeightMultiplier(getSpawnEggOffspringHeight());
        return thrush;
    }
    ...
    public boolean canMate(Animal p_30392_) {
        if (p_30392_ == this) {
            return false;
        } else if (!(p_30392_ instanceof LaughingthrushEntity)) {
            return false;
        } else {
            LaughingthrushEntity swallow = (LaughingthrushEntity)p_30392_;
    ...
    public Ingredient getBirdFood() {
        return Ingredient.of(CreaturesItems.MEALWORMS.get(), Items.SWEET_BERRIES);
    }
