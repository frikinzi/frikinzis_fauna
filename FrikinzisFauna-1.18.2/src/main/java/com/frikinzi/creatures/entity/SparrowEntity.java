...
public class SparrowEntity extends CreaturesFlyingBirdEntity implements IAnimatable, IAnimationTickable {
    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
    public static Map<Integer, TranslatableComponent> SPECIES_NAMES = ImmutableMap.of(
            1, new TranslatableComponent("message.creatures.eurasiantree"),
            2, new TranslatableComponent("message.creatures.sudangolden"),
            3, new TranslatableComponent("message.creatures.desertsparrow"),
            4, new TranslatableComponent("message.creatures.saffronbilled"),
            5, new TranslatableComponent("message.creatures.house"),
            6, new TranslatableComponent("message.creatures.chipping")
    );

    public SparrowEntity(EntityType<? extends SparrowEntity> p_29362_, Level p_29363_) {
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<SparrowEntity>(this, "controller", 0, this::predicate));
    }
    ...
    public static boolean checkBirdSpawnRules(EntityType<SparrowEntity> p_29424_, LevelAccessor p_29425_, MobSpawnType p_29426_, BlockPos p_29427_, Random p_29428_) {
    ...
    public SparrowEntity getBreedOffspring(ServerLevel p_149088_, AgeableMob p_149089_) {
        SparrowEntity fairywren = CreaturesEntities.SPARROW.get().create(p_149088_);
        fairywren.setVariant(this.getVariant());
        fairywren.setGender(this.random.nextInt(2));
        fairywren.setHeightMultiplier(getSpawnEggOffspringHeight());
        return fairywren;
    }
    ...
    public boolean canMate(Animal p_30392_) {
        if (p_30392_ == this) {
            return false;
        } else if (!(p_30392_ instanceof SparrowEntity)) {
            return false;
        } else {
            SparrowEntity lovebird = (SparrowEntity)p_30392_;
