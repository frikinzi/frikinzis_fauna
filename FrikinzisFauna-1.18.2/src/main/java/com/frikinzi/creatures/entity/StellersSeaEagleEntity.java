...
public class StellersSeaEagleEntity extends RaptorEntity implements IAnimatable, IAnimationTickable {
    ...
    public StellersSeaEagleEntity(EntityType<? extends StellersSeaEagleEntity> p_29362_, Level p_29363_) {
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<StellersSeaEagleEntity>(this, "controller", 0, this::predicate));
    }
    ...
    public static boolean checkBirdSpawnRules(EntityType<StellersSeaEagleEntity> p_29424_, LevelAccessor p_29425_, MobSpawnType p_29426_, BlockPos p_29427_, Random p_29428_) {
        return p_29425_.getBlockState(p_29427_.below()).is(BlockTags.PARROTS_SPAWNABLE_ON) && isBrightEnoughToSpawn(p_29425_, p_29427_);
    }

    public StellersSeaEagleEntity getBreedOffspring(ServerLevel p_149088_, AgeableMob p_149089_) {
        StellersSeaEagleEntity stellersseaeagle = CreaturesEntities.STELLERS_SEA_EAGLE.get().create(p_149088_);
        stellersseaeagle.setGender(this.random.nextInt(2));
        stellersseaeagle.setHeightMultiplier(getSpawnEggOffspringHeight());
        return stellersseaeagle;
    }
    ...
    public boolean canMate(Animal p_30392_) {
        if (p_30392_ == this) {
            ...
            return false;
        } else if (!(p_30392_ instanceof StellersSeaEagleEntity)) {
            return false;
        } else {
            StellersSeaEagleEntity stellersseaeagle = (StellersSeaEagleEntity)p_30392_;
            if (!stellersseaeagle.isTame()) {
                return false;
            } else if (stellersseaeagle.isInSittingPose()) {
                return false;
            } else {
                return this.isInLove() && stellersseaeagle.isInLove();
            }
        }
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (!this.isSleeping()) {
            return CreaturesSound.STELLERS_SEA_EAGLE_AMBIENT; } else {
            return null;
        }
    }
    ...
    public Set<Item> getTameFood() {
        return Sets.newHashSet(Items.COD, Items.SALMON, CreaturesItems.RAW_TROUT.get());
    }
