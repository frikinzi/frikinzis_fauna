...
public class GyrfalconEntity extends RaptorEntity implements IAnimatable, IAnimationTickable {
    ...
    public GyrfalconEntity(EntityType<? extends GyrfalconEntity> p_29362_, Level p_29363_) {
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<GyrfalconEntity>(this, "controller", 0, this::predicate));
    }
    ...
    public static boolean checkBirdSpawnRules(EntityType<GyrfalconEntity> p_29424_, LevelAccessor p_29425_, MobSpawnType p_29426_, BlockPos p_29427_, Random p_29428_) {
        return p_29425_.getBlockState(p_29427_.below()).is(BlockTags.PARROTS_SPAWNABLE_ON) && isBrightEnoughToSpawn(p_29425_, p_29427_);
    }

    public GyrfalconEntity getBreedOffspring(ServerLevel p_149088_, AgeableMob p_149089_) {
        GyrfalconEntity gyrfalcon = CreaturesEntities.GYRFALCON.get().create(p_149088_);
        gyrfalcon.setGender(this.random.nextInt(2));
        gyrfalcon.setHeightMultiplier(getSpawnEggOffspringHeight());
        return gyrfalcon;
    }
    ...
    public boolean canMate(Animal p_30392_) {
        if (p_30392_ == this) {
            ...
            return false;
        } else if (!(p_30392_ instanceof GyrfalconEntity)) {
            return false;
        } else {
            GyrfalconEntity gyrfalcon = (GyrfalconEntity)p_30392_;
            if (!gyrfalcon.isTame()) {
                return false;
            } else if (gyrfalcon.isInSittingPose()) {
                return false;
            } else {
                return this.isInLove() && gyrfalcon.isInLove();
            }
        }
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (!this.isSleeping()) {
            return CreaturesSound.GYRFALCON_AMBIENT; } else {
            return null;
        }
    }
