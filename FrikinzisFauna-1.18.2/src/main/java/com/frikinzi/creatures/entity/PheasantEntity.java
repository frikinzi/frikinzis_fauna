...
public class PheasantEntity extends CreaturesBirdEntity implements IAnimatable, IAnimationTickable {
    private AnimationFactory factory = GeckoLibUtil.createFactory(this);
    public static Map<Integer, TranslatableComponent> SPECIES_NAMES = ImmutableMap.of(
            1, new TranslatableComponent("message.creatures.goldenpheasant"),
            2, new TranslatableComponent("message.creatures.ladyamherst"),
            3, new TranslatableComponent("message.creatures.silverpheasant")
    );

    public PheasantEntity(EntityType<? extends PheasantEntity> p_29362_, Level p_29363_) {
    ...
    public void registerControllers(AnimationData data) {
        data.addAnimationController(new AnimationController<PheasantEntity>(this, "controller", 0, this::predicate));
    }
    ...
    public static boolean checkBirdSpawnRules(EntityType<PheasantEntity> p_29424_, LevelAccessor p_29425_, MobSpawnType p_29426_, BlockPos p_29427_, Random p_29428_) {
    ...
    public PheasantEntity getBreedOffspring(ServerLevel p_149088_, AgeableMob p_149089_) {
        PheasantEntity pheasant = CreaturesEntities.PHEASANT.get().create(p_149088_);
        pheasant.setVariant(this.getVariant());
        pheasant.setGender(this.random.nextInt(2));
        pheasant.setHeightMultiplier(getSpawnEggOffspringHeight());
        return pheasant;
    }
    ...
    public boolean canMate(Animal p_30392_) {
        if (p_30392_ == this) {
            return false;
        } else if (!(p_30392_ instanceof PheasantEntity)) {
            return false;
        } else {
            PheasantEntity pheasant = (PheasantEntity)p_30392_;
            return this.isInLove() && pheasant.isInLove();
        }
    }
    ...
    protected SoundEvent getAmbientSound() {
        if (!this.isSleeping()) {
            return CreaturesSound.PHEASANT_AMBIENT; } else {
            return null;
        }
    }

    public double getHatchChance() {
        return CreaturesConfig.pheasant_hatch_chance.get();
    }

    public int getClutchSize() {
        return this.random.nextInt(CreaturesConfig.pheasant_clutch_size.get());
    }
